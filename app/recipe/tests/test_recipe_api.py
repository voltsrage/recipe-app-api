...
class PrivateRecipeAPITests(TestCase):
	...
	def test_retrieve_recipes(self):
		"""Test retrievinf a list of recipes"""
	...
	def test_full_update(self):
			...
			recipe = create_recipe(
				...
				description= 'Sample recipe first description',
			)
	...
	def test_create_recipe_with_new_ingredients(self):
		...
		payload ={
			...
			'time_minutes': 30,
			'price': Decimal('1.50'),
			'ingredients': [{'name':'Chili'},{'name':'Mustard'}]
		}
	...
	def test_create_recipe_with_existing_ingredients(self):
		"""Test creating a recipe with new ingredients."""
		ingredient_indian = Ingredient.objects.create(user=self.user, name='Celery')
		payload ={
			'title':'Pongal',
			'time_minutes': 60,
			'price': Decimal('1.50'),
			'ingredients': [{'name':'Celery'},{'name':'Avocado'}]
		}
	...
	def test_create_ingredient_on_update(self):
		...
		payload = {'ingredients': [{'name':'Halepeno'}]}
		url = detail_url(recipe.id)
		res = self.client.patch(url,payload,format='json')

		self.assertEqual(res.status_code,status.HTTP_200_OK)
		new_ingredient = Ingredient.objects.get(user=self.user, name='Halepeno')
	...
	def test_update_recipe_assign_ingredient(self):
		...
		ingredient_spinach = Ingredient.objects.create(user=self.user, name='Spinach')
		payload = {'ingredients': [{'name':'Spinach'}]}
		url = detail_url(recipe.id)
		res = self.client.patch(url,payload,format='json')

		self.assertEqual(res.status_code,status.HTTP_200_OK)
		self.assertIn(ingredient_spinach,recipe.ingredients.all())
